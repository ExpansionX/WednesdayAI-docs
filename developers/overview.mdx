...
<CardGroup cols={2}>
  ...
  <Card title="Context engine hooks" href="/developers/context-engine" icon="layers">
    Collect, project, and prune per-turn context. `api.on("context.collect", ...)` and friends.
  </Card>
...
3. **No fork-base pin** — bundled extensions carry no `openclaw` peer dependency; compatibility is expressed by our SemVer version and the `forkBase` marker. Third-party plugins target our published SemVer (ADR 0005).
