...
**Baileys dependency is lazy-loaded.** The `whatsapp` bundled extension pins `@whiskeysockets/baileys` `7.0.0-rc14` and loads it lazily at channel-activation time, not at gateway startup. That release depends on registry `libsignal` (not a git-protocol spec) and on `whatsapp-rust-bridge` `0.5.4`. Plugins that interact with the WhatsApp channel must not assume the Baileys library is available as a direct import — use only the `openclaw/plugin-sdk` surface.
