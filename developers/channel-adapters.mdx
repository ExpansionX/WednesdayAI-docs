...
| `elevated` | `ChannelElevatedAdapter` | Fallback allow-from list when none is configured |
...
| `agentPrompt` | `ChannelAgentPromptAdapter` | Message-tool prompt hints |
...
- **Third-party installable plugins** put `openclaw` in `devDependencies` targeting our published SemVer, with `"openclaw": "*"` in `peerDependencies` — see the [Plugin SDK installation pattern](/developers/sdk#installation). Never put `openclaw` in `dependencies`; the gateway provides the SDK at runtime.
