...
Set `agents.list[].tools.profile` to apply a baseline tool set:

- `minimal` — read, write, edit, exec only
- `coding` — coding-oriented tools
- `messaging` — messaging-oriented tools
- `full` — all tools

Example: global coding profile + messaging-only agent:
...
### Additive allowlists (`alsoAllow`)

Use `tools.alsoAllow` or `agents.list[].tools.alsoAllow` to add tool entries without replacing an existing allowlist or profile:
...
`tools.byProvider` and `agents.list[].tools.byProvider` scope tool policy to a specific model provider or `provider/model` pair:
...
```json5
{
  tools: {
    byProvider: {
      openai: { deny: ["browser"] },
      "openrouter/deepseek/deepseek-chat-v3-0324": { allow: ["read", "exec"] },
    },
  },
}
```

### Exec configuration per agent

`agents.list[].tools.exec` overrides exec tool defaults for that agent:
...
### Filesystem restrictions per agent

`agents.list[].tools.fs.workspaceOnly: true` restricts filesystem tools to the agent workspace directory.
