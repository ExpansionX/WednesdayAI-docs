...
- Reads from the configured session storage backend (`fs-jsonl`, `sqlite`, or `postgres`) — not
  only from JSONL files. See [Session Management / External session storage](/concepts/session#external-session-storage).
...
- After completion, OpenClaw runs a sub-agent **announce step** and posts the result to the requester chat channel.
  - The announce step reads the sub-agent transcript from the configured session storage backend,
    so the output is consistent whether the backend is JSONL, SQLite, or Postgres.
