...
const { fetch: stubFetch, requests } = createAuthCaptureJsonFetch({
  response: { text: "transcribed audio" },
});

// pass stubFetch as request.fetchFn, invoke your handler, then assert:
