...
| `before_file_write` | Before any plugin file write to the filesystem | Yes — return `{ content }` to rewrite, or `{ skip: true }` to abort |
| `session_start` | A session starts | No (observe) |
| `session_end` | A session ends | No (observe) |
| `subagent_spawning` | A subagent is about to spawn | Yes — `status: "ok" \| "error"`; may also return `systemPrompt`, `model`, `maxTokens`, `tools`, or `skip: true` to modify or cancel the spawn |
...
### `before_file_write` example

```typescript
api.on("before_file_write", (payload) => {
