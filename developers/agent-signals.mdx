---
title: "Agent signals"
description: "Publish structured non-user stimuli into WednesdayAI via api.runtime.signals: context hints, heartbeat wakes, notifications, and background turns."
audience: developer
---
...
- `notify` — delivers the signal summary as an outbound notification to its `deliveryTarget`, respecting visibility suppression and active hours. It does not wake a model turn.
- `background` — runs a background agent turn for the target session, deferring while lanes are busy when `requiresIdle: true`.

The `steer` mode is reserved for future plumbing and returns `unsupported-mode`.
...
| `mode` | `"context" \| "heartbeat" \| "notify" \| "background" \| "steer"` | Dispatch mode; `steer` is rejected with `unsupported-mode`. |
...
- Use `context` mode for passive context; `heartbeat` mode when the agent should consider acting soon; `notify` to deliver a notification without a turn; `background` to run a turn outside the user conversation.
