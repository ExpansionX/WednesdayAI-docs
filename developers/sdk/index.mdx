...
- **Hook payload types** — `PluginHookMessageReceivedEvent`, `PluginHookBeforeContextSendEvent`, `PluginHookStorageAfterAppendEvent`. Note: `PluginHookSubagentSpawningEvent` is **not** exported from `openclaw/plugin-sdk`; use `import type` from core types directly, or derive the type with the `Parameters<...>` pattern.
...
interface RealtimeVoiceProviderPlugin {
  ...
  createBridge(req: { userId: string; channelId: string }): RealtimeVoiceBridge
}
```
...
// Append a single line (fire-and-forget):
appendFileLog(line: string, options: { path: string }): void
