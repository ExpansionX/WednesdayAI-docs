...
(The name can be a short alias your admin has set, or a full model id like `provider/model-name` - or just send `/model` to see what is available.)
...
- **Voice** - if your admin has enabled it, ask for spoken replies with `/tts on` (on Discord the command is `/voice`).
