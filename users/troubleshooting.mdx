...
## I saw a weird "task" or "dream" trace in Langfuse

Those are **not** chat replies. When your admin enables OpenTelemetry, background maintenance (the task pipeline) and optional "dream cycle" memory work leave short traces named like `openclaw.task.completed` or `openclaw.dream.cycle_completed`. An error-looking span named `openclaw.task.recovery_exhausted` means the system stopped retrying a stuck background job — it does not mean your chat failed. Ask your admin if something you expected never finished.
