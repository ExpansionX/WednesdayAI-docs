...
```json5
{
  agents: {
    defaults: {
      ...
      nested: {
        maxConcurrent: 2, // coordination lane cap for nested/orchestrator work (default: 2)
      },
    },
  },
}
...
- If the gateway RPC channel fails while waiting for a sub-agent to complete, the run is finalized with `status: error` and a message indicating the wait failure, rather than silently remaining in an active state.
...
- `tools.subagents.model`: default model selection for spawned sub-agents (string or `{ primary, fallbacks }`).
- `tools.subagents.tools.allow`: if set, becomes allow-only (deny still wins).
- `tools.subagents.tools.alsoAllow`: additional allowlist entries merged into allow and/or default sub-agent denylist.
- `tools.subagents.tools.deny`: denied tools (deny wins over allow).
...
- Nested/orchestrator coordination uses the `nested` lane with
  `agents.defaults.nested.maxConcurrent` (default `2`)
...
- If a sub-agent's `agent.wait` call fails (e.g., gateway RPC channel drops), the run is finalized as an error completion with cleanup, rather than remaining stuck as "active" until the next gateway restart.
