...
- `tools.exec.backgroundMs`: default time (ms) before an exec command auto-backgrounds.
- `tools.exec.timeoutSec`: default timeout (seconds) before auto-killing exec commands.
- `tools.exec.cleanupMs`: how long to keep finished sessions in memory (ms).
- `tools.exec.notifyOnExitEmptySuccess`: also emit success exit notifications when a backgrounded exec has no output (default: false).
