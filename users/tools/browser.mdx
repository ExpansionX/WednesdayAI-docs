...
- `remoteCdpTimeoutMs` applies to remote (non-loopback) CDP reachability checks (default: 1500ms).
- `remoteCdpHandshakeTimeoutMs` applies to remote CDP WebSocket reachability checks (default: max(remoteCdpTimeoutMs \* 2, 2000ms)).
...
- `extraArgs` appends extra Chrome launch flags (e.g. `["--window-size=1920,1080", "--disable-infobars"]`).
- `snapshotDefaults.mode: "efficient"` uses compact role snapshots by default when the caller does not specify a mode.
- `evaluateEnabled: false` disables `act:evaluate` (arbitrary JS execution in the browser). Default: `true`. Set to `false` for stricter security posture.
...
Set `browser.snapshotDefaults.mode` to control the default snapshot style when the agent or CLI does not specify a mode:

```json5
{
  browser: {
    snapshotDefaults: {
      mode: "efficient", // compact role snapshot (interactive + compact + depth)
    },
  },
}
```

When set to `"efficient"`, the browser tool uses a compact role snapshot by default. Without this setting, the default is an AI snapshot with numeric refs.
...
Each profile in `browser.profiles.<name>` supports:

- `cdpPort` — CDP port for local managed profiles (auto-assigned if omitted)
- `cdpUrl` — CDP URL for remote profiles (e.g. `"http://10.0.0.42:9222"`)
- `driver` — profile driver: `"openclaw"` (default, managed browser) or `"extension"` (Chrome extension relay)
- `attachOnly` — per-profile override: never launch, only attach to existing browser
- `color` — accent color (hex) for UI tinting
