...
Lifecycle plugin hooks are the `api.on(...)` surface defined by the `PluginHookName` union in `src/plugins/types.ts` — model resolution, prompt build, LLM input/output, tool calls, message and session lifecycle, subagents, gateway start/stop, and the `context.*` / `compaction.plan` context-engine hooks. The full catalogue with event shapes and mutation semantics lives in the [Hooks developer guide](/developers/hooks#lifecycle-hook-catalogue); the hook below is one mutating example from that set.
