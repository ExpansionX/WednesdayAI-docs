...
The binary is also available as `wednesdayai` — both names are equivalent.
...
Key flags: `--section <section>` — run only the specified wizard section instead of the full wizard. Valid values: `workspace | model | web | gateway | daemon | channels | skills | health`.
...
Foreground run flags: `--port <port>`, `--bind <loopback\|tailnet\|lan\|auto\|custom>`, `--token <token>`, `--auth <none\|token\|password\|trusted-proxy>`, `--password <password>`, `--tailscale <off\|serve\|funnel>`, `--allow-unconfigured`, `--force`, `--verbose`, `--ws-log <auto\|full\|compact>`.
...
`models status` key flags: `--check`, `--probe` (validates credentials live — does not send a completion), `--probe-timeout <ms>` (probe timeout, default `10000`).
