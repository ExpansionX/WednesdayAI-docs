...
`models status` key flags: `--check`, `--probe` (validates credentials live — does not send a completion), `--probe-timeout <ms>` (probe timeout, default `10000`), `--probe-provider <name>` (probe only the named provider), `--probe-profile <id>` (filter to specific profile(s); repeatable or comma-separated), `--probe-concurrency <n>` (number of concurrent probe requests), `--probe-max-tokens <n>` (max tokens for probe completion request).
