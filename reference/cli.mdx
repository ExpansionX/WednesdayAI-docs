...
openclaw update check            # check for a newer version (exit 0=up-to-date, exit 1=update available)
openclaw update changelog        # print the changelog for the version you would update to
```
...
openclaw models status --probe           # send a test completion to verify the model is reachable (not just configured)
openclaw models status --probe --timeout 15000   # timeout for probe request (default: 10000 ms)
...
`models status` key flags: `--check`, `--probe` (live completion probe, consumes tokens), `--timeout <ms>` (probe timeout, default `10000`).
...
openclaw channels capabilities whatsapp                      # list capabilities: reactions, threads, voice, files
openclaw channels resolve telegram <external-user-id>        # resolve external user ID to an OpenClaw user record
```

Subcommands: `list`, `status`, `logs`, `add`, `remove`, `login`, `logout`, `capabilities <channel>`, `resolve <channel> <external-user-id>`. Common flags: `--channel <name>`, `--account <id>`, `--name <label>`, `--json`. `--channel` accepts `whatsapp\|telegram\|discord\|googlechat\|slack\|mattermost\|signal\|imessage\|msteams` (plus installed channel extensions). `channels add` runs a wizard when no flags are passed; flags switch to non-interactive mode.
...
openclaw sessions cleanup --older-than 30 --dry-run   # preview stale record pruning
openclaw sessions cleanup --older-than 90 --force     # prune without confirmation
...
`sessions cleanup` flags: `--older-than <days>` (default: `90`), `--dry-run` (preview only; no changes made), `--force` (skip confirmation prompt).

`sessions backfill-user-ids` is safe on live data and idempotent — re-running it is a no-op for records already migrated.
...
```bash
openclaw session-storage status                           # show store type, connection health, and record counts
openclaw session-storage migrate --from sqlite --to redis # migrate session data between backends
openclaw session-storage vacuum                           # run VACUUM on the SQLite database (no-op for other backends)
```

Subcommands: `status`, `migrate --from <type> --to <type>`, `vacuum`. Common flags: `--json`.
