...
`models status` key flags: `--check`, `--probe` (validates credentials live — does not send a completion), `--timeout <ms>` (probe timeout, default `10000`).
...
| `--fix-missing` | Repair records with missing required fields |
