...
```json5
{
  gateway: {
    ...
    controlUi: {
      enabled: true,
      basePath: "/",
      root: false,                        // serve control UI at root path
...
| `gateway.controlUi.root` | boolean | `false` | Serve the control UI at the root path (`/`) rather than under `basePath` |
...
```json5
{
  secrets: {
    providers: {
      "env": { source: "env" },
      "vault": { source: "exec", command: "vault-fetch" },
    },
...
```json5
{
  acp: {
    stream: {
      ...
      maxOutputChars: 0,
      maxSessionUpdateChars: 0,
      tagVisibility: "default",
    },
...
| `acp.stream.hiddenBoundarySeparator` | string | `""` | Separator inserted at hidden chunk boundaries |
| `acp.stream.maxOutputChars` | integer | `0` | Max total output characters (0 = unlimited) |
| `acp.stream.maxSessionUpdateChars` | integer | `0` | Max chars per session update (0 = unlimited) |
| `acp.stream.tagVisibility` | string/object | `"default"` | Controls tag visibility in the stream |
| `acp.dispatch` | object | — | Dispatch sub-object (see extended config) |
| `acp.backend` | object | — | Backend sub-object (see extended config) |
...
```json5
{
  diagnostics: {
    ...
    capture: { mode: "off" },           // off | sampled | all
    cacheTrace: false,
    modelPayloadLog: false,             // true | false | path string
...
| `langfuse.sessionIdleTimeoutMs` | integer | — | Idle timeout (ms) before a Langfuse session is closed |

### Top-level `diagnostics` keys

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `diagnostics.capture` | object | — | Session capture config: `{ mode: "off" \| "sampled" \| "all", schedule?, ... }` |
| `diagnostics.cacheTrace` | boolean | `false` | Log cache trace events for debugging prompt-cache behaviour |
| `diagnostics.modelPayloadLog` | boolean \| string | `false` | Log raw model request/response payloads. Set to `true` (stderr) or a file path |
...
<Warning>
`modelPayloadLog` writes unredacted model payloads to disk or stderr. Never enable in production — logs will contain user messages and model responses in plaintext.
</Warning>
