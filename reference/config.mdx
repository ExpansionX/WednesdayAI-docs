...
```json5
{
  session: {
    storage: {
      ...
      cache: "none",           // "none" | "redis" — redis cache requires backend "postgres"
      redisUrl: "",            // redis:// connection string (redis cache only)
      redis: { keyPrefix: "ctx", ttlSeconds: 300 },
    },
    maintenance: {
      mode: "warn",              // "enforce" | "warn"
      pruneAfter: "30d",         // prune entries older than this ("30d", "12h", or days number)
      maxEntries: 500,           // max session entries
      rotateBytes: "10mb",       // rotate sessions store after this size
      // resetArchiveRetention: "30d",  // defaults to pruneAfter; false disables
      // maxDiskBytes: "500mb",         // per-agent disk budget; enforce cleans oldest-first
      // highWaterBytes: "400mb",       // target after cleanup; default 80% of maxDiskBytes
    },
  },
}
```
...
| `session.storage.cache` | string | `"none"` | `"none" \| "redis"` — Redis read-through cache for session context. `"redis"` requires `backend: "postgres"` and `session.storage.redisUrl`; otherwise the gateway logs a warning and continues without a cache |
| `session.storage.redisUrl` | string | — | Redis connection string (redis cache only) |
| `session.storage.redis.keyPrefix` | string | `"ctx"` | Prefix applied to Redis cache keys |
| `session.storage.redis.ttlSeconds` | integer | `300` | Time-to-live for Redis cache entries (seconds) |
| `session.maintenance.mode` | string | `"warn"` | `"enforce" \| "warn"` — warn only, or enforce cleanup limits |
| `session.maintenance.pruneAfter` | string \| number | `"30d"` | Age at which session entries are pruned (e.g. `"30d"`, `"12h"`, or a day count) |
| `session.maintenance.maxEntries` | integer | `500` | Max session entries to keep |
| `session.maintenance.rotateBytes` | integer \| string | `"10mb"` | Rotate the sessions store when it exceeds this size |
| `session.maintenance.resetArchiveRetention` | string \| number \| `false` | same as `pruneAfter` | Retention for archived reset transcripts; `false` disables cleanup |
| `session.maintenance.maxDiskBytes` | integer \| string | — | Per-agent sessions-directory disk budget (e.g. `"500mb"`). When exceeded: warn (`mode=warn`) or oldest-first cleanup (`mode=enforce`) |
| `session.maintenance.highWaterBytes` | integer \| string | 80% of `maxDiskBytes` | Target size after disk-budget cleanup (e.g. `"400mb"`) |
