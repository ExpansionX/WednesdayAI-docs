...
Controls the gateway's network listener addresses.
...
|-----|------|---------|-------------|
| `network.host` | string | `"0.0.0.0"` | Bind address for all network listeners |
| `network.port` | integer | `3000` | Primary HTTP port (deprecated alias: `port`) |
| `nodeHost` | string | — | URL of this node as seen by other nodes in a multi-gateway mesh. Example: `"https://node1.example.com"` |
...
| `gateway.restartPolicy` | `"on-failure"` | No |
| `gateway.restartResilience` | `3` | No |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `gateway.restartPolicy` | string | `"on-failure"` | `"always" \| "on-failure" \| "never"` — controls auto-restart behaviour |
| `gateway.restartResilience` | integer | `3` | Consecutive crash-restarts allowed before giving up (0–10) |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `session.storage` | string | `"sqlite"` | `"memory" \| "sqlite" \| "postgres"` — backing store for session data |
| `session.maintenance.enabled` | boolean | `true` | Enables periodic housekeeping (pruning old sessions) |
| `session.maintenance.intervalHours` | number | `24` | How often maintenance runs |
| `session.maintenance.maxAgeDays` | number | `90` | Sessions older than this are pruned |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `browser.ssrfPolicy.allowPrivateRanges` | boolean | `false` | Allow the browser to fetch RFC-1918 private addresses |
| `browser.ssrfPolicy.allowedHosts` | string[] | `[]` | Additional hostnames to allow |
| `browser.ssrfPolicy.blockedHosts` | string[] | `[]` | Hostnames to explicitly block |
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `diagnostics.slowRequestMs` | integer | `5000` | Threshold (ms) for logging slow requests |
| `diagnostics.traceLevel` | string | `"errors"` | `"off" \| "errors" \| "all"` |
