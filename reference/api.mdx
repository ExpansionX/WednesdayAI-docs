...
```json
{ "type": "event", "event": "<name>", "payload": { }, "seq": 42, "stateVersion": { "presence": 7, "health": 3 } }
```

`seq` is a monotonically increasing broadcast sequence; `stateVersion` is present on state-snapshot events (`presence`, `health`). Slow consumers that cannot keep up are disconnected (close code `1008`); discardable events are dropped instead.

| Event | Payload | Emitted when |
| ----- | ------- | ------------|
| `connect.challenge` | `{ nonce, ts }` | Immediately on connection (see handshake above) |
| `agent` | `{ runId, seq, stream, ts, sessionKey?, data }` | Agent run streaming updates (assistant deltas, tool events, lifecycle phases) |
| `chat` | `{ runId, sessionKey, seq, state, message?, errorMessage? }` where `state` is `delta` \| `final` \| `error` | Webchat run progress and final messages |
| `presence` | `{ presence: [...] }` | System presence snapshot changes |
| `tick` | `{ ts }` | Periodic keepalive |
| `health` | Health summary snapshot | Health refresh; sensitive fields are redacted unless the client has admin scope |
| `heartbeat` | Heartbeat event | After each heartbeat run; redacted unless admin scope |
| `cron` | Cron job lifecycle event (e.g. `{ action: "finished", jobId }`) | Cron job state changes |
| `talk.mode` | `{ enabled, phase, ts }` | Voice talk mode changes |
| `talk.realtime.relay` | Realtime voice relay frames | During realtime voice sessions |
| `shutdown` | `{ reason, restartExpectedMs }` | Gateway is shutting down |
| `node.pair.requested` | Pairing request | A node requests pairing (requires the pairing scope) |
| `node.pair.resolved` | Pairing resolution | A node pairing request is approved/rejected (pairing scope) |
| `node.invoke.request` | Invocation request | The gateway dispatches a command to a connected node |
| `device.pair.requested` | Device pairing request | A device requests pairing (pairing scope) |
| `device.pair.resolved` | Device pairing resolution | A device pairing request is approved/rejected (pairing scope) |
| `voicewake.changed` | `{ triggers }` | Voice wake-word triggers change |
| `exec.approval.requested` | Approval request | An exec approval is requested (approvals scope) |
| `exec.approval.resolved` | Approval resolution | An exec approval decision is made (approvals scope) |
| `sessions.runsUpdate` | `{ runs, capturedAt, stuckThresholdMs }` | Active session run state changes |
| `update.available` | `{ updateAvailable: { currentVersion, latestVersion, channel } \| null }` | Gateway update availability changes |
...
| Sessions | `sessions.list`, `sessions.preview`, `sessions.patch`, `sessions.reset`, `sessions.delete`, `sessions.compact`, `sessions.thresholds`, `sessions.interrupt`, `sessions.claims.status` |
...
| System state | `last-heartbeat`, `set-heartbeats`, `system-presence`, `system-event` — heartbeat and presence RPCs behind the `openclaw system` CLI (flat method names; there is no `system.*` prefix) |
| Plugins | `plugins/health` — current plugin health report (status, per-plugin diagnostics) |
