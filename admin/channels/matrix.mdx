...
<Note>
`channels.matrix.rooms` is a legacy alias for `groups` — same shape, retained for backwards compatibility. Use `groups` in new configs; `rooms` is accepted for users migrating from older setups.
</Note>

<Note>
**Power level requirements:** OpenClaw needs minimum power level **0** to send messages. To send reactions, power level 0 is sufficient provided the `m.reaction` event type is allowed in the room. To send state events (e.g. changing the room topic), power level **50** is required.
</Note>
...
For fine-grained control, use the `e2ee` object instead of the shorthand `encryption: true`:
...
channels:
  matrix:
    e2ee:
      enabled: true
      storePath: "~/.openclaw/matrix-crypto.db"
      verifyDevices: false
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `e2ee.enabled` | boolean | `true` | Enable E2EE for encrypted rooms |
| `e2ee.storePath` | string | `"~/.openclaw/matrix-crypto.db"` | SQLite store for crypto keys |
| `e2ee.verifyDevices` | boolean | `false` | Refuse to decrypt from unverified devices |

<Warning>
If `storePath` is lost or corrupted, encrypted history becomes unreadable. Back up this file.
</Warning>
...
Matrix Spaces let rooms be organised into a tree. Support is experimental.
...
channels:
  matrix:
    spaces:
      enabled: false
      autoJoinChildren: false
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `spaces.enabled` | boolean | `false` | Participate in Matrix Spaces (experimental) |
| `spaces.autoJoinChildren` | boolean | `false` | Auto-join all rooms within a joined Space |
