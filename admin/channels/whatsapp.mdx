...
- Node.js 22+ on the gateway host
...
<Note>
If you installed WednesdayAI globally via npm, the install path for the extension is `$(npm root -g)/wednesdayai/extensions/whatsapp`. Run `npm install --omit=dev` there if the automated install fails.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="deps missing / PLUGIN_DEPS_MISSING warning">
    Run `openclaw plugins install whatsapp` to reinstall the plugin and its dependencies. If that fails, navigate to the extension directory and run `npm install --omit=dev` manually.
  </Accordion>
</AccordionGroup>
...
High-signal fields: `dmPolicy`, `allowFrom`, `groupPolicy`, `groupAllowFrom`, `groups`, `textChunkLimit`, `chunkMode`, `mediaMaxMb`, `sendReadReceipts`, `ackReaction`, `historyLimit`, `actions`, `configWrites`, `accounts.<id>.*`, `web.enabled`, `web.heartbeatSeconds`, `web.reconnect.*`
