...
- `channels.synology-chat.enabled`: enable/disable channel startup.
- `channels.synology-chat.token`: outgoing webhook secret token.
- `channels.synology-chat.incomingUrl`: incoming webhook URL for sending messages.
- `channels.synology-chat.webhookPath`: webhook path (default: `/webhook/synology`).
- `channels.synology-chat.dmPolicy`: `pairing | allowlist | open | disabled`.
- `channels.synology-chat.allowedUserIds`: DM allowlist (Synology user IDs).
- `channels.synology-chat.rateLimitPerMinute`: rate limit per sender (default: 30).
- `channels.synology-chat.allowInsecureSsl`: allow self-signed NAS certs (default: false).
- `channels.synology-chat.accounts`: multi-account configuration.
