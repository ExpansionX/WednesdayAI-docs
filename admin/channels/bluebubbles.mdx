...
- `channels.bluebubbles.chunkMode`: `length` (default) splits only when exceeding `textChunkLimit`; `newline` prefers paragraph/block boundaries and packs as many complete paragraphs as fit into each message up to `textChunkLimit`, falling back to length-based splitting only when a single block exceeds the limit. Fenced code blocks and tables are kept intact.
- `channels.bluebubbles.chunkNewlinePacking`: packing mode for `newline` chunking (default: `true`). Set to `false` to restore one message per paragraph.
...
- `channels.bluebubbles.configWrites`: Allow channel-initiated config writes (default: true).
- `channels.bluebubbles.ackReaction`: Per-channel ack reaction emoji override.
- `channels.bluebubbles.ackReactionScope`: Scope for ack reactions (default: `group-mentions`).
- `channels.bluebubbles.responsePrefix`: Per-channel outbound response prefix override.
