...
- `channels.slack.historyLimit`: max channel messages fetched as history context (default `50`; set `0` to disable). Falls back to `messages.groupChat.historyLimit` when unset.
- `channels.slack.dmHistoryLimit`: max DM turns kept as history context; per-user override via `channels.slack.dms["<userId>"].historyLimit`.

Reply threading controls:

- `channels.slack.replyToMode`: `off|first|all` (default `off`)
- `channels.slack.replyToModeByChatType`: per-chat-type overrides keyed by `direct|group|channel`, each `off|first|all`. When set for a chat type, it takes precedence over `replyToMode` and over the legacy `dm.replyToMode` fallback
...
<AccordionGroup>
  ...
  <Accordion title="Outbound text and files">
    ...
    - `channels.slack.chunkMode="newline"` prefers paragraph/block boundaries and packs as many complete paragraphs as fit into each message up to `textChunkLimit`, falling back to length-based splitting only when a single block exceeds the limit. Fenced code blocks and tables are kept intact.
    - `channels.slack.chunkNewlinePacking` (default `true`): set to `false` to restore one message per paragraph (the pre-packing behaviour).
...
Slack supports per-channel reaction notification mode:

- `channels.slack.reactionNotifications`: `off | own | all | allowlist` (default: `own`)
- `off`: ignore all reactions
- `own`: notify when users react to bot-sent messages
- `all`: notify agent of all reactions
- `allowlist`: notify only for reactions from users in the channel allowlist

Reaction events still respect Slack access controls; unauthorized senders are dropped.
...
- [Configuration reference - Slack](/gateway/configuration-reference#slack)
  ...
  - delivery: `textChunkLimit`, `chunkMode`, `chunkNewlinePacking`, `mediaMaxMb`, `streaming`, `nativeStreaming`
  - ops/features: `configWrites`, `commands.native`, `slashCommand.*`, `actions.*`, `userToken`, `userTokenReadOnly`, `reactionNotifications`, `ackReaction`, `responsePrefix`, `heartbeat`
