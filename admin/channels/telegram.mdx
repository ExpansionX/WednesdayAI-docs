...
Per-account override: `channels.telegram.accounts.<id>.dmPolicy` and `accounts.<id>.allowFrom` override channel-level defaults for that account. Named accounts do **not** inherit `accounts.default.allowFrom` — they inherit `channels.telegram.allowFrom` when account-level values are unset.
