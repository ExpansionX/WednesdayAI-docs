...
- `channels.msteams.chunkMode`: `length` (default) or `newline` to pack paragraph blocks up to `textChunkLimit` per message; oversized blocks fall back to length splitting.
- `channels.msteams.chunkNewlinePacking`: packing mode for `newline` chunking (default: `true`). Set to `false` to restore one message per paragraph.
