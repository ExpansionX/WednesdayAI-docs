...
- `channels.zalouser.enabled`: enable/disable channel startup.
- `channels.zalouser.dmPolicy`: `pairing | allowlist | open | disabled` (default: pairing).
- `channels.zalouser.allowFrom`: DM allowlist (user IDs, lowercase).
- `channels.zalouser.groupPolicy`: `allowlist | open | disabled` (default: allowlist).
- `channels.zalouser.groupAllowFrom`: group sender allowlist.
- `channels.zalouser.groups`: per-group config keyed by group ID.
- `channels.zalouser.accounts`: multi-account configuration.
- `channels.zalouser.textChunkLimit`: outbound chunk size (chars, default 2000).
