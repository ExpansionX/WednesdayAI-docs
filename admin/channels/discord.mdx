...
<Warning>MESSAGE_CONTENT is a privileged intent. It must be enabled in the Discord Developer Portal for bots in 100+ servers.</Warning>
...
OpenClaw registers a `/wednesday` slash command automatically on startup. Use `commandScope` to control where the command is registered:

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `commandScope` | `"global" \| "guild"` | `"guild"` | Registration scope for the `/wednesday` command |
| `guildId` | string | — | Required when `commandScope: "guild"` |
...
<Warning>Global command registration can take up to 1 hour to propagate. Use `guild` scope during setup.</Warning>
...
channels:
  discord:
    threads:
      ...
      createForLongReplies: false
```

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `threads.enabled` | boolean | `true` | Participate in threads |
| `threads.autoArchiveDuration` | integer | `1440` | `60 \| 1440 \| 4320 \| 10080` minutes |
| `threads.createForLongReplies` | boolean | `false` | Auto-create thread when reply exceeds 2000 chars |
...
Controls which Discord interaction component types the bot uses in responses:
...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `components.buttons` | boolean | `true` | Enable button components in responses |
| `components.selectMenus` | boolean | `true` | Enable select menu components |
| `components.modals` | boolean | `false` | Enable modal (form) interactions |
