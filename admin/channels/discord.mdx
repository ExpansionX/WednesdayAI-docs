...
<Steps>
  ...
  <Step title="Enable privileged intents">
    ...
    The bot always requests the Message Content intent. Server Members and Presence are only requested when you also set their config flags (`channels.discord.intents.guildMembers` / `channels.discord.intents.presence`); enabling them in the Portal alone is not enough.
...
<AccordionGroup>
  ...
  <Accordion title="Privileged intents">
    ...
    OpenClaw config for privileged intents:
    ...
    ```json5
    {
      channels: {
        discord: {
          intents: {
            presence: false,      // Guild Presences intent (requires Portal opt-in)
            guildMembers: false,  // Guild Members intent (requires Portal opt-in)
          },
        },
      },
    }
...
```
  ...
  <Accordion title="Ack reactions">
    ...
    Scope control via `channels.discord.ackReactionScope`:

    - `group-mentions` — only in group chats when mentioned
    - `group-all` — in all group chat messages
    - `direct` — only in DMs
    - `all` — in all chats
    - `off` / `none` — disabled
...
```
  ...
  <Accordion title="Reaction notifications">
    Discord supports per-guild reaction notification mode:

    - `channels.discord.guilds.<guildId>.reactionNotifications`: `off | own | all | allowlist` (default: `own`)
    - `off`: ignore all reactions
    - `own`: notify when users react to bot-sent messages
    - `all`: notify agent of all reactions
    - `allowlist`: notify only for reactions from users in the guild allowlist
...
```
    ...
    Reaction events still respect Discord access controls (guild policy, allowlists); unauthorized senders are dropped.
...
```
  ...
  <Accordion title="Long-running handlers time out or duplicate replies">
    ...
    Additional event queue tuning:

    - `channels.discord.eventQueue.maxQueueSize` — max events queued before backpressure (default: 10000)
    - `channels.discord.eventQueue.maxConcurrency` — max concurrent event processing operations (default: 50)
...
- event queue: `eventQueue.listenerTimeout`, `eventQueue.maxQueueSize`, `eventQueue.maxConcurrency`
- reply/history: `replyToMode`, `historyLimit`, `dmHistoryLimit`, `dms.*.historyLimit`
- delivery: `textChunkLimit`, `chunkMode`, `chunkNewlinePacking`, `maxLinesPerMessage`
- streaming: `streaming` (legacy alias: `streamMode`), `draftChunk`, `blockStreaming`, `blockStreamingCoalesce`
- media/retry: `mediaMaxMb`, `retry`
- actions: `actions.*`
- presence: `activity`, `status`, `activityType`, `activityUrl`
- UI: `ui.components.accentColor`
- intents: `intents.presence`, `intents.guildMembers`
- features: `pluralkit`, `execApprovals`, `intents`, `agentComponents`, `heartbeat`, `responsePrefix`, `ackReaction`, `ackReactionScope`, `proxy`, `guilds.*.reactionNotifications`
