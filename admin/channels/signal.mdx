...
channels:
  signal:
    trustOnSafetyNumberChange: false
```

<Warning>
Safety number changes (when a contact reinstalls Signal) cause message delivery failures. These are logged as `signal.safety_number_changed` events. With `trustOnSafetyNumberChange: false` (default), manual trust confirmation is required.
</Warning>
