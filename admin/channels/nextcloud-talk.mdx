...
- `channels.nextcloud-talk.chunkMode`: `length` (default) or `newline` — registered but not effective on the current Nextcloud Talk delivery path (the plugin uses a direct send, not the shared dispatcher).
- `channels.nextcloud-talk.chunkNewlinePacking`: registered for future compatibility; currently has no effect (see `chunkMode` note above).
