...
<Tabs>
  ...
  <Tab title="npm (manual)">
    ...
    The package provides both `wednesdayai` and `openclaw` binaries (permanent alias); this guide uses `openclaw`.
...
<Accordion title="pnpm global install">
  ...
  pnpm approve-builds -g    # select wednesdayai, sharp, node-llama-cpp when prompted
  ```
</Accordion>
...
<Accordion title="sharp build errors on macOS">
  ...
  ```bash
  SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g wednesdayai@latest
  ```
</Accordion>
