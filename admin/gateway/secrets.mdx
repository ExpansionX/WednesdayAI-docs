...
- Timeout failures settle the provider request once. If the child process produces no stdout before
  `noOutputTimeoutMs` or exceeds `timeoutMs`, resolution fails with an actionable timeout error and
  late output from the child process is ignored rather than producing a second result.
