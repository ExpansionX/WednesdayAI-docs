...
When [workspace lanes](/concepts/workspace-lanes) are enabled on an agent, add `fenceToLane: true`
to the sandbox config to bind the sandbox tool filesystem to the user's lane workspace instead of
the shared persona root. Without this flag, a sandboxed multi-lane agent can still traverse up to
the persona root and read sibling lane directories via the workspace mount.
...
`workspaceAccess: "rw"` is required for complete isolation. With `"none"` or `"ro"`, the sandbox
uses a scope-keyed directory seeded from the lane directory; two sessions sharing the same scope key
can cross-read each other's seeded files. A warning is logged at session setup when `fenceToLane:
true` and `workspaceAccess` is not `"rw"`.

`fenceToLane` is only applied when workspace lanes are enabled, the lane is resolved, and the policy
decision is `"allowed"`. It has no effect in single-user configurations or when the lane is denied.
The fence restricts tool filesystem access only; bootstrap resolution remains lane-aware and can
still select lane-local files such as `AGENTS.md`, with persona fallback where applicable.
