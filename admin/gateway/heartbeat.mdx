...
- **Cron** — a job with a `wakeGate` payload (see
  [Cron vs Heartbeat](/automation/cron-vs-heartbeat#nudge-and-wake-gate-jobs)).
- **Heartbeat** — a heartbeat configured with a `wakeGate` (next section).
- **Plugin / hook** — `api.runtime.signals.nudge(...)` (see
  [Agent Signals](/plugins/agent-signals#nudge)).
- **CLI** — [`openclaw agent nudge`](/cli/agent-nudge).
