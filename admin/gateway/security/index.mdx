...
If `--fix` cannot safely rewrite an invalid config file, it records that config
mutation was skipped and still continues with filesystem hardening such as state
directory and config-file permission repair. Fix the reported config syntax or
schema issues, then rerun `openclaw security audit --fix` if a config migration or
policy rewrite was also needed.
...
4. Claim is not listed in [Out of Scope](https://github.com/ExpansionX/WednesdayAI-core/blob/main/SECURITY.md#out-of-scope).
...
Workspace lanes are a separate opt-in workspace-file boundary for multi-user-capable personas. Use
them with secure DM mode when one persona may serve multiple users, but do not treat them as a
hostile co-tenant security boundary. See [Workspace Lanes](/concepts/workspace-lanes).
