...
### Multi-user persona workspace lanes

Use workspace lanes only for agents that may serve multiple people. Pair this with secure DM mode:
`session.dmScope` separates conversation history, while `workspaceLanes` separates lane-local
workspace files such as `USER.md` and memory files.
...
See [Workspace Lanes](/concepts/workspace-lanes) for current scope and limitations. This is an
opt-in substrate, not a complete hosted multi-tenant isolation system.
