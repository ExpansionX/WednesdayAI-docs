...
- Operators may call `agent.nudge` (`operator.admin`) to inject non-user context into an agent's
  main session and request a targeted heartbeat wake. The request schema is `AgentNudgeParams`:
  `text` is required; optional fields are `data`, `agentId`, `sessionKey`, `reason`, `source`,
  `dedupeKey`, `ttlMs`, `priority`, and `requiresIdle`. Extra fields are rejected. A successful
  response is `AgentNudgeResult` with `{ "ok": true, "sessionKey": "...", "signalId": "..." }`.
