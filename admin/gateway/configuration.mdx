...
<AccordionGroup>
  ...
  <Accordion title="Configure session storage backend">
    By default, session transcripts are stored as local JSONL files. For durable or shared storage,
    ...
    ```json5
    {
      session: {
        storage: {
          ...
          sqlitePath: "~/.openclaw/agents/{agentId}/sessions/session.db",
          fallbackToJsonlOnError: false,
          migration: { validateOnStart: true },
        },
      },
    }
    ```
    ...
    ```json5
    {
      session: {
        storage: {
          backend: "postgres",
          mode: "primary",
          databaseUrl: "${SESSION_DATABASE_URL}",
          fallbackToJsonlOnError: false,
          cache: "redis",
          redisUrl: "${REDIS_URL}",
          migration: { validateOnStart: true },
    ...
    - `backend` selects the single source of truth for transcript data. JSONL files are touched only
      when `backend: "fs-jsonl"`.
    - `mode`: `primary` means the selected backend is the sole writer. `mirror` keeps JSONL as a side
      mirror while the selected backend serves reads.
    - `fallbackToJsonlOnError`: when `true`, a failing database write falls back to JSONL for that
      single write. This preserves availability but splits state; use with caution.
    - `migration.validateOnStart`: when `true`, the gateway validates schema and may backfill legacy
      JSONL content on first startup. Set to `false` for a clean database cut-over.
    - SQLite options under `storage.sqlite`: `wal`, `busyTimeoutMs`, `walAutocheckpointPages`.
    - Redis options under `storage.redis`: `keyPrefix`, `ttlSeconds`.

    See [Session Management / External session storage](/concepts/session#external-session-storage)
    for the configured-backend-only invariant and sub-agent output behavior.
