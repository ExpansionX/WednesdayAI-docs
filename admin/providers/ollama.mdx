...
- Sets `maxTokens` to `8192` (fixed default)
- Sets all costs to `0`
...
For auto-discovered models, OpenClaw uses the context window reported by Ollama when available, otherwise it defaults to `128000`. The default `maxTokens` for all Ollama models is `8192`. You can override both in explicit provider config:
