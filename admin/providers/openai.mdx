...
Built-in `openai/*` models use the OpenAI **Responses** API (`openai-responses`); `openai-codex/*` uses `openai-codex-responses`. For custom OpenAI-compatible endpoints under `models.providers`, set `api` to one of `openai-completions` (chat completions), `openai-responses`, or `openai-codex-responses`.
