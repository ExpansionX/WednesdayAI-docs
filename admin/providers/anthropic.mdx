...
**Best for:** scripted or containerized setups where you already hold an Anthropic OAuth token (`sk-ant-oat-...`) and want to inject it via environment variable without running `claude setup-token`.
...
```json5
{
  env: { ANTHROPIC_OAUTH_TOKEN: "sk-ant-oat-..." },
  agents: { defaults: { model: { primary: "anthropic/claude-opus-4-6" } } },
}
```

**Priority:** `ANTHROPIC_OAUTH_TOKEN` is checked before `ANTHROPIC_API_KEY`. If both are set, the OAuth token wins.
